...
| Uno sfondo a sorpresa da tutto l'archivio | `…/w/island?date=casuale` — cambia da solo ogni giorno, comodo in automazione |
...
| `GET /w/<flusso>[.jpg\|.png][?date=\|?v=][&dl=1]` *(pubblico)* | immagine del giorno per la lock screen; il corpo è **sempre** byte immagine, **mai** JSON — se il canale non ha ancora generato nulla o `?date=` non esiste, risponde con il placeholder statico. `?date=casuale` pesca invece un giorno a caso da tutto l'archivio del canale, deterministico per la giornata (stessa data per tutto il giorno, `cache-control: no-store` come la chiamata senza data). Con `?dl=1` la risposta aggiunge `content-disposition: attachment` con un nome file parlante (`artipop-<flusso>-<data>.png`/`.jpg`), per far arrivare il salvataggio su disco con un nome riconoscibile invece del blob senza estensione — non emesso sul placeholder |
...
| `GET /canali.opml` *(pubblico)* | elenco OPML 2.0 dei feed per canale: un `<outline type="rss">` per ogni flusso attivo con `xmlUrl` verso `/feed/<flusso>.xml` — si importa in blocco nel lettore di feed invece di incollare un indirizzo alla volta; in caso di errore resta un OPML valido col `<body>` vuoto, mai JSON |
| `GET /promemoria.ics` *(pubblico)* | calendario iCalendar (RFC 5545) sottoscrivibile: un evento giornaliero ricorrente alle 03:00 UTC che ricorda il wallpaper nuovo, con link al canale; parametro opzionale `?c=<flusso>` per il promemoria di un canale specifico (un valore sconosciuto dà il promemoria generico, mai un errore) |
