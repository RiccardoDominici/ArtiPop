...
IPHONE_WALLPAPER_HEIGHT = 3328

def build_iphone_wallpaper_prompt(user_prompt: str) -> str:
    """
    Build an optimized prompt for iPhone lock/home screen wallpapers.
    Keeps the subject centered, leaves clean space for clock/Dynamic Island,
    and avoids artifacts like text/watermarks.
    """
    additions = (
        "iPhone wallpaper, portrait, center composition, subject centered, "
        "clean gradient background, minimal detail in the top 20% and bottom 10%, "
        "no text, no watermark, no borders, no UI, no logo, "
        "high-quality, crisp, aesthetic, lock screen friendly"
    )
    if user_prompt:
        return f"{user_prompt.strip()}, {additions}"
    return additions
...
def generate_image_with_replicate(
    ...
    raw_ar = f"{width//g}:{height//g}"

    # Replicate SD3.5 Large only allows a fixed set of aspect ratios
    allowed_ars = {"16:9", "1:1", "21:9", "2:3", "3:2", "4:5", "5:4", "9:16", "9:21"}

    if raw_ar in allowed_ars:
        ar = raw_ar
    else:
        ar = "9:16" # default to portrait if unsupported
...
def create_filename_with_date(prefix: str = "sd3") -> str:
    """
    Generate a filename with tomorrow's date.
    Example: sd3_2025-10-14.png
    """
...
def create_s3_key_organized(base_path: str = "images") -> str:
    """
    Create an S3 key organized by year/month/day using tomorrow's date.
    Example: images/2025/10/14/sd3_2025-10-14.png
    """
...
def upload_to_s3(
    ...
    # Merge optional tags into S3 metadata
    if tags:
        try:
            s3_metadata.update({str(k): str(v) for k, v in tags.items()})
        except Exception:
            pass
...
def main():
    ...
    parser.add_argument("--public", action="store_true", help="Make image publicly accessible via URL")
    parser.add_argument("--iphone", action="store_true", default=True, help="Optimize for iPhone wallpapers (portrait ~6:13 ≈ 1536x3328) and enhance the prompt")
    ...
    try:
        ...
        # iPhone wallpaper mode: lock dimensions and enhance prompt
        if args.iphone:
            logger.info("iPhone wallpaper mode enabled: forcing portrait ~6:13 (1536x3328) and enhancing prompt")
            args.width = IPHONE_WALLPAPER_WIDTH
            args.height = IPHONE_WALLPAPER_HEIGHT
            final_prompt = build_iphone_wallpaper_prompt(args.prompt)
        else:
        ...
        # Prepare extra tags for S3 if iPhone wallpaper mode is active
        extra_tags = None
        if args.iphone:
            extra_tags = {"wallpaper": "iphone", "aspect": "6:13", "mode": "iphone_wallpaper"}
        ...
        if args.iphone:
            print(f"🎯 Mode:        iPhone wallpaper (portrait ~6:13)")
            print(f"🖼️ Resolution:  {args.width}x{args.height}")
